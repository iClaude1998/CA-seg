...
class Bioparse_segmentation_amos22(Dataset):
    """
    ...
    def produce_sample_list(self):
        ...
        for mask_name in self.mask_name_list:
            prefix, suffix = os.path.splitext(mask_name)
            adir, mask_name = os.path.split(prefix)
            
            split = adir.split('_')[1]
    ...
    def __getitem__(self, index) -> Dict[str, Any]:
        ...
        intermap = np.load(f"{self.root_dir}/{self.modality}/{self.intermap_name_list[index]}")
