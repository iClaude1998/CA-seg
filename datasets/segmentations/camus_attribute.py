...
class camusattributes_seg(Dataset):
    r"""
    ...
    def __getitem__(self, index) -> Dict[str, Any]:
        ...
        for key in prompt:
            if prompt[key] is None:
                prompt[key] = 'no'
        # skin melanoma
        input_prompts = [f"{prompt['location']}",
                         f"{prompt['shape']} {prompt['location']}", 
                         f"{prompt['size']} {prompt['location']}", 
                         f"{prompt['shape']} {prompt['size']} {prompt['location']}",
                         f"{prompt['size']} {prompt['shape']} {prompt['location']}",
                         f"a picture of {prompt['location']}",
                         ]
        if isinstance(sentense_prompt, str):
            input_prompts.extend([sentense_prompt])
        elif isinstance(sentense_prompt, list):
            input_prompts.extend(sentense_prompt) if len(sentense_prompt) == 5 else input_prompts.extend(sentense_prompt + ['no polyp' for _ in range(5-len(sentense_prompt))])
