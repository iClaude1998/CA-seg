...
#SBATCH --time=08:00:00         # time allocation, which has the format (D-HH:MM), here set to 1 hour
...
config_file=configs/flowmatch/bioparse/radiography1.yaml
learn_obj=recflow


python main.py --task thresh_search \
               --exp_name ${exp_name} \
               --config ${config_file} \
               --device cuda \
               --learn_obj ${learn_obj} \
