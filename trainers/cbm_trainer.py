    ...
    def train(self, gradient_accumulation_steps=1):
        ...
        for epoch in range(self.start_epoch, self.num_epoch):
            ...
            for iter_id, batch in tqdm(enumerate(self.train_dataloader),desc=f'Epoch {epoch}'):
                ...
                loss = self.criterion(preds, sdf_maps)
        ...
        for epoch in range(self.start_epoch, self.num_epoch):
            ...
            for iter_id, batch in tqdm(enumerate(self.train_dataloader, desc=f'Epoch {epoch}')):
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    concept_weights = self.model(images)
                    preds = torch.sum(concept_weights[..., None, None] * cams, dim=1)
                    loss = self.criterion(preds, sdf_maps)
    ...
    def validation(self):
        ...
        for batch in tqdm(self.val_dataloader, desc='Validation'):
            ...
            with torch.no_grad():
                ...
                preds = postprocess_pred(preds)
                loss = self.criterion(preds, sdf_maps)
                iou_batch = compute_metrics(preds, sdf_maps, mask_name, metric='iou', thresh=33, gt_type='sdf_map')
                dice_batch = compute_metrics(preds, sdf_maps, mask_name, metric='dice', thresh=33, gt_type='sdf_map')
    ...
    def test(self, testset='test'):
        ...
        for batch in tqdm(dl, desc='Test'):
            ...
            with torch.no_grad():
                ...
                untrained = postprocess_pred(untrained)
                preds = postprocess_pred(preds)
                iou_batch_I = compute_metrics(untrained, sdf_maps, mask_name, metric='iou', thresh=33, gt_type='sdf_map')
                dice_batch_I = compute_metrics(untrained, sdf_maps, mask_name, metric='dice', thresh=33, gt_type='sdf_map')
                
                iou_batch_II = compute_metrics(preds, sdf_maps, mask_name, metric='iou', thresh=33, gt_type='sdf_map')
                dice_batch_II = compute_metrics(preds, sdf_maps, mask_name, metric='dice', thresh=33, gt_type='sdf_map')
