...
from .adapters import Generic_UNet_v1
...
class UNetModel_v3position(nn.Module):
    ...
    def __init__(
        ...
        if norm_type == 'instancenorm':
            self.hwm = nn.InstanceNorm2d(1)
        elif norm_type == 'batchnorm':
            self.hwm = nn.BatchNorm2d(1)
        else:
            self.hwm = nn.Identity()
    ...
    def forward(self, h, timesteps, c):
        """
        ...
        if c.shape[-2:] != h.shape[-2]:
            uemb = F.interpolate(c, size=(hf, wf), mode='bilinear', align_corners=False)
        uemb = self.hwm(uemb)
